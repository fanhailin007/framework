...
本文档描述当前后端用户管理模块的设计。内容基于已经落地的代码和 `users` 表结构，不包含菜单管理、角色权限分配或完整登录认证流程。

## 1. 目标

用户管理模块用于维护后台用户账号，提供基础增删改查能力，并为后续登录、权限、会话管理提供用户数据基础。
...
| 登录认证 | 未实现，后续仍放入 `UserController` |
| 角色分配 | 未实现，后续由角色权限模块处理 |
...
| 登录安全字段 | `failed_login_count`、`locked_until`、`last_login_at`、`last_login_ip` 已在表结构中预留 |

当前尚未实现认证拦截、接口鉴权和登录态校验。后续登录认证能力落地后，仍应将用户相关 HTTP 入口集中在 `UserController`。
...
| 尚未实现登录接口 | 后续在 `UserController` 中增加登录、登出、刷新 Token |
| 尚未实现接口鉴权 | 后续结合角色权限模块和认证模块处理 |
...
用户管理模块当前已经形成 Controller、Service、Mapper、Entity、DTO 的完整基础闭环。API 入口集中在 `UserController`，业务规则集中在 `UserManagementServiceImpl`，数据库访问由 MyBatis-Plus `UserMapper` 承担。

后续应优先补齐登录认证能力，并保持“用户相关 HTTP API 统一放入 `UserController`”这一约束不变。
